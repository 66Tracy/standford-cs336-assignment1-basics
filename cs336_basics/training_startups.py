...
class BPETrainer:
    ...
    def merge_bytepair_to_source_word(self, word_tokenizing:list):
        """从一个分词状态，反推出原单词"""
        source_word = bytes(word_tokenizing[0][0]).decode("utf-8") + bytes(word_tokenizing[0][1]).decode("utf-8")
        for i in range(1,len(word_tokenizing)):
            source_word += bytes(word_tokenizing[i][1]).decode("utf-8")
    ...
    def merge_and_update_bytepair(self, most_frequent_pair: Tuple[bytes, bytes]):
        """根据传入的bytepair"""
        # 先对自身做更新
        new_token = bytes([most_frequent_pair[0]]) + bytes([most_frequent_pair[1]])

        # 源词的序列列表
        source_word_index_list = self.bytepair_from_words_index[most_frequent_pair]
        new_source_word_index_list = []
        for index in source_word_index_list:
            i, j = index[0], index[1]
            # 获取源词的分词状态
            word_tokenizing = self.words_tokenizing_states[i]
            # 需要合并词元，反推出当前词的个数
            ...
            if j >= 1:
                ...
                new_left_pair = (bytes([left_pair[0]]), new_token)
                self.bytepair_counting_dict[new_left_pair] += source_word_count
                # 修改左边的pair的倒查索引
                left_pair_source_word_index_list = self.bytepair_from_words_index[left_pair]
                target_idx = 0
                while target_idx < len(left_pair_source_word_index_list):
                    if left_pair_source_word_index_list[target_idx][0] == i and left_pair_source_word_index_list[target_idx][1] == j-1:
                        break
                    target_idx += 1
                left_pair_source_word_index_list.pop(target_idx)
                self.bytepair_from_words_index[left_pair] = left_pair_source_word_index_list

            if j <= len(word_tokenizing)-2:
                right_pair = word_tokenizing[j+1]
                self.bytepair_counting_dict[right_pair] -= source_word_count
                new_right_pair = (new_token, bytes([right_pair[1]]))
                self.bytepair_counting_dict[new_right_pair] += source_word_count
                # 修改右边pair的倒查索引
                right_pair_source_word_index_list = self.bytepair_from_words_index[right_pair]
                target_idx = 0
                while target_idx < len(right_pair_source_word_index_list):
                    if right_pair_source_word_index_list[target_idx][0] == i and right_pair_source_word_index_list[target_idx][1] == j-1:
                        break
                    target_idx += 1
                right_pair_source_word_index_list.pop(target_idx)
                self.bytepair_from_words_index[right_pair] = right_pair_source_word_index_list

            # 更新word_tokenizing
            if j >= 1:
                word_tokenizing[j-1] = new_left_pair
                self.bytepair_from_words_index[new_left_pair].append((i,j-1))
            if j <= len(word_tokenizing)-2:
                word_tokenizing[j+1] = new_right_pair
                self.bytepair_from_words_index[new_right_pair].append((i,j))
            word_tokenizing.pop(j)
            self.words_tokenizing_states[i] = word_tokenizing
    ...
    def train_bpe(self):
        ...
        for word, counts in self.words_counting.items():
            ...
            word_split = [char_byte for char_byte in word_encoding]
            ...
            for i in range(len(word_split)-1):
                ...
                self.bytepair_from_words_index[byte_pair].append((len(self.words_tokenizing_states),len(word_tokenizing)-1))
        ...
        while TABLE_SIZE <= self.vocab_size:
            ...
            print(f"Iter-{iter_time}, most frequent pairs: ({bytes([most_frequent_pairs[0]])},{bytes([most_frequent_pairs[1]])}), count: {max_count}")
...
if __name__ == "__main__":
    # obj = BPETrainer(input_path="datasets/TinyStoriesV2-GPT4-valid.txt", vocab_size=1000, special_tokens=["<|endoftext|>"])
